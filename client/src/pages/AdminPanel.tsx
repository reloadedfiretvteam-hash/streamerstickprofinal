...
export default function AdminPanel() {
  ...
  const [contactSearch, setContactSearchTerm] = useState("");
  const [selectedContactIds, setSelectedContactIds] = useState<Set<string>>(new Set());
  const [marketingCampaigns, setMarketingCampaigns] = useState<any[]>([]);
  const [loadingCampaigns, setLoadingCampaigns] = useState(false);
  const [marketingView, setMarketingView] = useState<'contacts' | 'campaigns' | 'campaign-editor' | 'campaign-detail'>('contacts');
  const [editingCampaign, setEditingCampaign] = useState<any>(null);
  const [viewingCampaign, setViewingCampaign] = useState<any>(null);
  const [campaignForm, setCampaignForm] = useState({ name: '', subject: '', bodyHtml: '', bodyText: '', segmentSource: 'all' });
  const [savingCampaign, setSavingCampaign] = useState(false);
  const [sendingCampaign, setSendingCampaign] = useState(false);
  const [previewCount, setPreviewCount] = useState<number | null>(null);
  const [sendToSelectedForm, setSendToSelectedForm] = useState<{ subject: string; bodyHtml: string; bodyText: string } | null>(null);
  ...
  const loadMarketingContacts = async () => {
    ...
    try {
      ...
      if (contactSourceFilter !== 'all') params.set('source', contactSourceFilter);
      if (contactSearch) params.set('search', contactSearch);
      const response = await authFetch(`/api/admin/marketing/contacts?${params}`);
      const result = await response.json();
      if (result.data) setMarketingContacts(result.data);
    } catch (error) {
  ...
  const loadMarketingCampaigns = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.data) setMarketingCampaigns(result.data);
    } catch (error) {
  ...
  const loadCampaignDetail = async (id: string) => {
    try {
      const response = await authFetch(`/api/admin/marketing/campaigns/${id}`);
      const result = await response.json();
      if (result.data) setViewingCampaign(result.data);
    } catch (error) {
  ...
  const saveCampaign = async () => {
    if (!campaignForm.name || !campaignForm.subject || !campaignForm.bodyHtml) {
      showToast('Name, subject, and HTML body are required', 'error');
      return;
    }
    setSavingCampaign(true);
    try {
      const segment = campaignForm.segmentSource !== 'all' ? { source: campaignForm.segmentSource } : undefined;
      const payload = {
        name: campaignForm.name,
        subject: campaignForm.subject,
        bodyHtml: campaignForm.bodyHtml,
        bodyText: campaignForm.bodyText || undefined,
        segment,
      };
      ...
      if (result.error) {
        ...
        showToast(editingCampaign?.id ? 'Campaign updated' : 'Campaign created', 'success');
        setMarketingView('campaigns');
        loadMarketingCampaigns();
        setEditingCampaign(null);
        setCampaignForm({ name: '', subject: '', bodyHtml: '', bodyText: '', segmentSource: 'all' });
      }
    } catch (error) {
      ...
      <aside className="w-64 border-r border-white/10 bg-gray-800 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <Button 
            ...
            className="w-full justify-start text-gray-300 hover:text-white hover:bg-white/5"
            onClick={() => { setActiveSection("email-marketing"); loadMarketingContacts(); loadMarketingCampaigns(); }}
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 mt-1">Manage contacts, campaigns, and sends</p>
                </div>
              </div>

              <div className="flex gap-2 border-b border-gray-700 pb-2">
                <Button
                  variant={marketingView === 'contacts' ? 'secondary' : 'ghost'}
                  size="sm"
                  onClick={() => { setMarketingView('contacts'); loadMarketingContacts(); }}
                >
                  <Users className="w-4 h-4 mr-2" /> Contacts
                </Button>
                <Button
                  variant={marketingView === 'campaigns' ? 'secondary' : 'ghost'}
                  size="sm"
                  onClick={() => { setMarketingView('campaigns'); loadMarketingCampaigns(); }}
                >
                  <Mail className="w-4 h-4 mr-2" /> Campaigns
                </Button>
                {(marketingView === 'campaign-editor') && (
                  <Button variant="secondary" size="sm">
                    <Edit className="w-4 h-4 mr-2" /> {editingCampaign?.id ? 'Edit' : 'New'} Campaign
                  </Button>
                )}
                {(marketingView === 'campaign-detail') && (
                  <Button variant="secondary" size="sm">
                    <Eye className="w-4 h-4 mr-2" /> Campaign Detail
                  </Button>
                )}
              </div>

              {/* ===== CONTACTS VIEW ===== */}
              {marketingView === 'contacts' && (
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-3 items-center">
                    <div className="relative flex-1 min-w-[200px]">
                      ...
                      <Input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') loadMarketingContacts(); }}
                        className="pl-10 bg-gray-800 border-gray-700 text-white"
                      />
                    </div>
                    <select
                      value={contactSourceFilter}
                      onChange={(e) => { setContactSourceFilter(e.target.value); }}
                    ...
                    <Button size="sm" onClick={loadMarketingContacts} className="bg-purple-600 hover:bg-purple-700">
                    ...
                    {selectedContactIds.size > 0 && (
                      <Button
                        ...
                        onClick={() => setSendToSelectedForm({ subject: '', bodyHtml: '', bodyText: '' })}
                      >
                        <Mail className="w-4 h-4 mr-1" /> Send to {selectedContactIds.size} selected
                      </Button>
                    )}
                  </div>
                  ...
                  <Card className="bg-gray-800 border-gray-700">
                    <Table>
                      <TableHeader>
                        <TableRow className="border-gray-700">
                          <TableHead className="text-gray-400 w-10">
                            <input
                              type="checkbox"
                              checked={marketingContacts.length > 0 && selectedContactIds.size === marketingContacts.length}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setSelectedContactIds(new Set(marketingContacts.map(c => c.id)));
                                } else {
                      ...
                      <TableBody>
                        {loadingContacts ? (
                          <TableRow><TableCell colSpan={7} className="text-center py-8 text-gray-400"><Loader2 className="w-6 h-6 animate-spin mx-auto" /></TableCell></TableRow>
                        ) : marketingContacts.length === 0 ? (
                          <TableRow><TableCell colSpan={7} className="text-center py-8 text-gray-400">No contacts found</TableCell></TableRow>
                          ...
                          <TableRow key={contact.id} className="border-gray-700 hover:bg-gray-700/50">
                            ...
                            <TableCell className="text-white font-mono text-sm">{contact.email}</TableCell>
                            <TableCell className="text-gray-300">{[contact.firstName, contact.lastName].filter(Boolean).join(' ') || '—'}</TableCell>
                            <TableCell>
                              <Badge className={contact.source === 'subscription' ? 'bg-green-600' : contact.source === 'firestick' ? 'bg-orange-600' : 'bg-purple-600'}>
                                {contact.source?.replace('_', ' ')}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-gray-400 text-sm">{contact.createdAt ? new Date(contact.createdAt).toLocaleDateString() : '—'}</TableCell>
                            <TableCell className="text-gray-400 text-sm">{contact.lastActivityAt ? new Date(contact.lastActivityAt).toLocaleDateString() : '—'}</TableCell>
                            <TableCell>
                              {contact.isSubscribed ? (
                                <Badge className="bg-green-600">Subscribed</Badge>
                              ) : (
                                <Badge className="bg-red-600">Unsubscribed</Badge>
                              )}
                            </TableCell>
                          </TableRow>
                        ))}
                  ...
                  <p className="text-gray-500 text-sm">{marketingContacts.length} contact(s) shown</p>

                  {/* Send to selected modal */}
                  {sendToSelectedForm && (
                    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
                      <div className="w-full max-w-2xl bg-gray-800 rounded-2xl">
                        <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-6 rounded-t-2xl flex items-center justify-between">
                          <h3 className="text-xl font-bold text-white">Send to {selectedContactIds.size} Contact(s)</h3>
                          <Button variant="ghost" onClick={() => setSendToSelectedForm(null)} className="text-white hover:bg-white/20"><X className="w-5 h-5" /></Button>
                        </div>
                        <div className="p-6 space-y-4">
                          <div>
                            <label className="block text-sm font-semibold text-gray-300 mb-1">Subject *</label>
                            <Input value={sendToSelectedForm.subject} onChange={(e) => setSendToSelectedForm({...sendToSelectedForm, subject: e.target.value})} className="bg-gray-700 border-gray-600 text-white" placeholder="Email subject line" />
                          </div>
                          <div>
                            <label className="block text-sm font-semibold text-gray-300 mb-1">HTML Body *</label>
                            <textarea value={sendToSelectedForm.bodyHtml} onChange={(e) => setSendToSelectedForm({...sendToSelectedForm, bodyHtml: e.target.value})} rows={8} className="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 font-mono text-sm" placeholder="<h1>Hello!</h1><p>Your email content...</p>" />
                          </div>
                          <div>
                            <label className="block text-sm font-semibold text-gray-300 mb-1">Plain Text (optional)</label>
                            <textarea value={sendToSelectedForm.bodyText} onChange={(e) => setSendToSelectedForm({...sendToSelectedForm, bodyText: e.target.value})} rows={3} className="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 text-sm" placeholder="Plain text fallback..." />
                          </div>
                        </div>
                        <div className="p-6 border-t border-gray-700 flex justify-end gap-3">
                          <Button variant="outline" onClick={() => setSendToSelectedForm(null)} className="border-gray-600 text-gray-300">Cancel</Button>
                          <Button
                            className="bg-purple-600 hover:bg-purple-700"
                            disabled={sendingCampaign || !sendToSelectedForm.subject || !sendToSelectedForm.bodyHtml}
                            onClick={async () => {
                              setSendingCampaign(true);
                              try {
                                const r = await authFetch('/api/admin/marketing/send-to-selected', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ contactIds: Array.from(selectedContactIds), ...sendToSelectedForm }),
                                });
                                const d = await r.json();
                                if (d.success) {
                                  showToast(`Sent: ${d.sent}, Failed: ${d.failed}`, 'success');
                                  setSendToSelectedForm(null);
                                  setSelectedContactIds(new Set());
                                } else {
                          ...
                          >
                            {sendingCampaign ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Mail className="w-4 h-4 mr-2" />}
                            Send Emails
                          </Button>
                        </div>
                      </div>
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setMarketingView('campaign-editor');
                      }}
                    >
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-gray-700">
                          ...
                          <TableHead className="text-gray-400">Failed</TableHead>
                      ...
                      <TableBody>
                        {loadingCampaigns ? (
                          ...
                          <TableRow><TableCell colSpan={7} className="text-center py-8 text-gray-400">No campaigns yet. Create one!</TableCell></TableRow>
                          ...
                          <TableRow key={campaign.id} className="border-gray-700 hover:bg-gray-700/50">
                            ...
                            <TableCell className="text-gray-300 text-sm truncate max-w-[200px]">{campaign.subject}</TableCell>
                            <TableCell>
                              <Badge className={campaign.status === 'sent' ? 'bg-green-600' : campaign.status === 'sending' ? 'bg-yellow-600' : 'bg-gray-600'}>
                                {campaign.status}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-green-400">{campaign.stats?.sent || 0}</TableCell>
                            <TableCell className="text-red-400">{campaign.stats?.failed || 0}</TableCell>
                            <TableCell className="text-gray-400 text-sm">{campaign.createdAt ? new Date(campaign.createdAt).toLocaleDateString() : '—'}</TableCell>
                            <TableCell>
                              <div className="flex gap-1">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setMarketingView('campaign-detail');
                                  }}
                                ...
                                {campaign.status === 'draft' && (
                                  <Button
                                    ...
                                    onClick={async () => {
                                      ...
                                      try {
                                        await authFetch(`/api/admin/marketing/campaigns/${campaign.id}`, { method: 'DELETE' });
              ...
              {marketingView === 'campaign-editor' && (
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    <Button variant="ghost" size="sm" onClick={() => setMarketingView('campaigns')} className="text-gray-400">
                  ...
                  <Card className="bg-gray-800 border-gray-700">
                    <CardHeader>
                      <CardTitle className="text-white">{editingCampaign?.id ? 'Edit Campaign' : 'New Campaign'}</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-semibold text-gray-300 mb-1">Campaign Name *</label>
                          <Input value={campaignForm.name} onChange={(e) => setCampaignForm({...campaignForm, name: e.target.value})} className="bg-gray-700 border-gray-600 text-white" placeholder="e.g., February Promotion" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-300 mb-1">Subject Line *</label>
                          <Input value={campaignForm.subject} onChange={(e) => setCampaignForm({...campaignForm, subject: e.target.value})} className="bg-gray-700 border-gray-600 text-white" placeholder="e.g., Exclusive 50% Off This Week!" />
                        </div>
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-300 mb-1">Segment (audience)</label>
                        <select
                          value={campaignForm.segmentSource}
                          onChange={(e) => setCampaignForm({...campaignForm, segmentSource: e.target.value})}
                          className="bg-gray-700 border border-gray-600 text-white rounded-md px-3 py-2 text-sm w-full"
                        >
                          <option value="all">All Contacts</option>
                          <option value="free_trial">Free Trial Users</option>
                          <option value="subscription">Subscription Customers</option>
                          <option value="firestick">Fire Stick Customers</option>
                        </select>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const segment = campaignForm.segmentSource !== 'all' ? campaignForm.segmentSource : '';
                              const r = await authFetch(`/api/admin/marketing/contacts/count?source=${segment}`);
                              const d = await r.json();
                              setPreviewCount(d.count ?? 0);
                            } catch { showToast('Failed to get count', 'error'); }
                          }}
                        >
                          Preview Count
                        </Button>
                        {previewCount !== null && <span className="text-gray-300 text-sm">{previewCount} subscribed recipients</span>}
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-300 mb-1">Email Body (HTML) *</label>
                        <textarea
                          value={campaignForm.bodyHtml}
                          onChange={(e) => setCampaignForm({...campaignForm, bodyHtml: e.target.value})}
                          rows={12}
                          className="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 font-mono text-sm"
                          placeholder={`<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">\n  <h1 style="color: #9333ea;">Special Offer!</h1>\n  <p>Hi there,</p>\n  <p>We have an exclusive deal for you...</p>\n  <a href="https://streamstickpro.com" style="background: #9333ea; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; display: inline-block;">Shop Now</a>\n</div>`}
                        />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-300 mb-1">Plain Text Body (optional)</label>
                        <textarea
                          value={campaignForm.bodyText}
                          onChange={(e) => setCampaignForm({...campaignForm, bodyText: e.target.value})}
                          rows={4}
                      ...
                      <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-700">
                        <Button onClick={saveCampaign} disabled={savingCampaign} className="bg-purple-600 hover:bg-purple-700">
                          {savingCampaign ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
                          {editingCampaign?.id ? 'Update Campaign' : 'Save as Draft'}
                        </Button>
                        {editingCampaign?.id && editingCampaign.status === 'draft' && (
                          <>
                            <Button
                              variant="outline"
                              className="border-blue-600 text-blue-400 hover:bg-blue-900/30"
                              onClick={async () => {
                                const testEmail = prompt('Enter email for test send:', 'reloadedfiretvteam@gmail.com');
                                if (!testEmail) return;
                                try {
                                  const r = await authFetch(`/api/admin/marketing/campaigns/${editingCampaign.id}/test-send`, {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ testEmail }),
                                  });
                                  const d = await r.json();
                                  showToast(d.message || d.error || 'Done', d.success ? 'success' : 'error');
                                } catch { showToast('Test send failed', 'error'); }
                              }}
                            >
                              <Mail className="w-4 h-4 mr-2" /> Test Send
                            </Button>
                            <Button
                              className="bg-green-600 hover:bg-green-700"
                              disabled={sendingCampaign}
                              onClick={async () => {
                                if (!confirm(`Send this campaign to all matching subscribed contacts? This cannot be undone.`)) return;
                                setSendingCampaign(true);
                                try {
                                  const r = await authFetch(`/api/admin/marketing/campaigns/${editingCampaign.id}/send`, { method: 'POST' });
                                  const d = await r.json();
                                  if (d.success) {
              ...
              {marketingView === 'campaign-detail' && viewingCampaign && (
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    <Button variant="ghost" size="sm" onClick={() => { setMarketingView('campaigns'); setViewingCampaign(null); }} className="text-gray-400">
                      &larr; Back to Campaigns
                    </Button>
                  </div>
                  ...
                  <Card className="bg-gray-800 border-gray-700">
                    <CardHeader>
                      <CardTitle className="text-white flex items-center gap-3">
                        ...
                        <Badge className={viewingCampaign.status === 'sent' ? 'bg-green-600' : viewingCampaign.status === 'sending' ? 'bg-yellow-600' : 'bg-gray-600'}>
                          {viewingCampaign.status}
                        </Badge>
                      </CardTitle>
                      <CardDescription className="text-gray-400">
                        Subject: {viewingCampaign.subject} &middot; Created: {viewingCampaign.createdAt ? new Date(viewingCampaign.createdAt).toLocaleString() : '—'}
                        {viewingCampaign.sentAt && <> &middot; Sent: {new Date(viewingCampaign.sentAt).toLocaleString()}</>}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      {viewingCampaign.stats && (
                        <div className="grid grid-cols-3 gap-4 mb-6">
                          <div className="bg-gray-700/50 p-4 rounded-lg text-center">
                            <p className="text-2xl font-bold text-blue-400">{(viewingCampaign.stats.queued || 0) + (viewingCampaign.stats.sent || 0) + (viewingCampaign.stats.failed || 0)}</p>
                            <p className="text-sm text-gray-400">Total</p>
                          </div>
                          <div className="bg-gray-700/50 p-4 rounded-lg text-center">
                            <p className="text-2xl font-bold text-green-400">{viewingCampaign.stats.sent || 0}</p>
                            <p className="text-sm text-gray-400">Sent</p>
                          </div>
                          <div className="bg-gray-700/50 p-4 rounded-lg text-center">
                            <p className="text-2xl font-bold text-red-400">{viewingCampaign.stats.failed || 0}</p>
                            <p className="text-sm text-gray-400">Failed</p>
                          </div>
                        </div>
                      )}

                      {viewingCampaign.sends && viewingCampaign.sends.length > 0 && (
                        <div>
                          <h4 className="text-white font-semibold mb-3">Send Log</h4>
                          <div className="max-h-96 overflow-y-auto">
                            <Table>
                              <TableHeader>
                                <TableRow className="border-gray-700">
                                  <TableHead className="text-gray-400">Contact ID</TableHead>
                                  <TableHead className="text-gray-400">Status</TableHead>
                                  <TableHead className="text-gray-400">Sent At</TableHead>
                                  <TableHead className="text-gray-400">Message ID</TableHead>
                                  <TableHead className="text-gray-400">Error</TableHead>
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                {viewingCampaign.sends.map((send: any) => (
                                  <TableRow key={send.id} className="border-gray-700">
                                    <TableCell className="text-gray-300 font-mono text-xs">{send.contactId?.substring(0, 8)}...</TableCell>
                                    <TableCell>
                                      <Badge className={send.status === 'sent' ? 'bg-green-600' : send.status === 'failed' ? 'bg-red-600' : 'bg-gray-600'}>
                                        {send.status}
                                      </Badge>
                                    </TableCell>
                                    <TableCell className="text-gray-400 text-sm">{send.sentAt ? new Date(send.sentAt).toLocaleString() : '—'}</TableCell>
                                    <TableCell className="text-gray-500 font-mono text-xs">{send.providerMessageId?.substring(0, 12) || '—'}</TableCell>
                                    <TableCell className="text-red-400 text-xs max-w-[200px] truncate">{send.errorMessage || '—'}</TableCell>
                                  </TableRow>
                                ))}
                              </TableBody>
                            </Table>
                      ...
                      <div className="mt-6 flex justify-end">
                        <Button variant="outline" size="sm" onClick={() => loadCampaignDetail(viewingCampaign.id)} className="border-gray-600 text-gray-300">
