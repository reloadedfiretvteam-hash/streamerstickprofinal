...
|------|--------------------|----------------|----------------|
| **25,000 pages** | 8K IPTV + 10K Jailbreak + 7K Google | **Seed in CI** | **Done:** `scripts/seed-25k-location-pages.ts` runs in deploy workflow after migrations. Set `VITE_SUPABASE_URL` + `SUPABASE_SERVICE_KEY` in GitHub Secrets; then sitemap has thousands of URLs. **Also fixed:** (1) Worker sitemap now paginates when reading `seo_architecture` (Supabase returns max 1000/request), so all 25K URLs can appear. (2) Workflow writes a “25K seed” summary so you see if it was SKIPPED (missing secrets) or RUN. See [SEO-URL-AUDIT.md](./SEO-URL-AUDIT.md). |
