-- ============================================================
-- FIX IMAGES TO PERSIST - Additional Tables for Section Images
-- Run this AFTER the main setup SQL
-- ============================================================

-- Table to store homepage section images (Hero, backgrounds, etc.)
CREATE TABLE IF NOT EXISTS section_images (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  section_name text UNIQUE NOT NULL, -- 'hero', 'carousel', etc.
  image_url text NOT NULL,
  image_type text DEFAULT 'background', -- 'background', 'overlay', etc.
  alt_text text,
...
-- ============================================================
-- FIX PRODUCT IMAGES - Ensure proper structure
-- ============================================================

-- Make sure product_images table has correct structure
DO $$
BEGIN
...
-- Make sure real_products table has main_image column
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
