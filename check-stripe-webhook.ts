 ...
 * 
 * This script will check your Stripe webhook configuration and tell you
 * which webhook is old (Supabase) vs new (Cloudflare).
 * 
 * Usage:
...
if (!STRIPE_SECRET_KEY) {
  ...
  console.error('  export STRIPE_SECRET_KEY="sk_live_YOUR_SECRET_KEY"');
  console.error('');
  console.error('Or on Windows PowerShell:');
  console.error('  $env:STRIPE_SECRET_KEY="sk_live_YOUR_SECRET_KEY"');
  console.error('');
  console.error('Then run: npx tsx check-stripe-webhook.ts');
  process.exit(1);
}
...
async function checkWebhooks() {
  ...
  try {
    ...
    if (error.type === 'StripeAuthenticationError') {
      console.error('');
      console.error('The Stripe secret key is incorrect or invalid.');
